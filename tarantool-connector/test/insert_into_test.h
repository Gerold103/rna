void test_insert_into()
{
	...
	Session ses;
	if (ses.Connect() == -1) {
		LogFL(DEBUG) << "test_insert_into(): connection failed, error code: " << ses.GetErrorCode() << ";\n";
		return;
	} else {
		LogFL(DEBUG) << "test_insert_into(): conected;\n";
	}

	DataStructure greet(128);
	LogFL(DEBUG) << "test_insert_into(): greet size: " << ses.Receive(greet) << "\n";
