void test_select_from()
{
	...
	Session ses;
	if (ses.Connect() == -1) {
		LogFL(DEBUG) << "test_select_from(): connection failed, error code: " << ses.GetErrorCode() << ";\n";
		return;
	} else {
		LogFL(DEBUG) << "test_select_from(): conected;\n";
	}

	DataStructure greet(128);
	LogFL(DEBUG) << "test_select_from(): greet size: " << ses.Receive(greet) << "\n";
