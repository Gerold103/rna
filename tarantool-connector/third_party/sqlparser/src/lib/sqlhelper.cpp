...
void inprint(const std::string &val, uint num_indent) { std::cout << indent(num_indent) << val << std::endl; }
...
void printTableRefInfo(TableRef* table, uint num_indent) {
  switch (table->type) {
    case kTableName:
      inprint(std::string(*(table->name)), num_indent);
      break;
    case kTableSelect:
...
void printImportStatementInfo(ImportStatement* stmt, uint num_indent) {
  ...
  inprint(std::string(*(stmt->table_name)), num_indent+1);
}
...
void printInsertStatementInfo(InsertStatement* stmt, uint num_indent) {
  ...
  if (stmt->columns != NULL) {
    ...
    for (std::string col_name : *stmt->columns) {
