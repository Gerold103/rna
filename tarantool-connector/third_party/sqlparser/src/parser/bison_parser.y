...
/* SQL Keywords */
%token <sval> DEALLOCATE PARAMETERS INTERSECT TEMPORARY TIMESTAMP
%token <sval> DISTINCT NVARCHAR RESTRICT TRUNCATE ANALYZE BETWEEN
%token <sval> CASCADE COLUMNS CONTROL DEFAULT EXECUTE EXPLAIN
%token <sval> HISTORY INTEGER NATURAL PREPARE PRIMARY SCHEMAS
%token <sval> SPATIAL VIRTUAL BEFORE COLUMN CREATE DELETE DIRECT
%token <sval> DOUBLE ESCAPE EXCEPT EXISTS GLOBAL HAVING IMPORT
%token <sval> INSERT ISNULL OFFSET RENAME SCHEMA SELECT SORTED
%token <sval> TABLES UNIQUE UNLOAD UPDATE VALUES AFTER ALTER CROSS
%token <sval> DELTA GROUP INDEX INNER LIMIT LOCAL MERGE MINUS ORDER
%token <sval> OUTER RIGHT TABLE UNION USING WHERE CALL DATE DESC
%token <sval> DROP FILE FROM FULL HASH HINT INTO JOIN LEFT LIKE
%token <sval> LOAD NULL PART PLAN SHOW TEXT TIME VIEW WITH ADD ALL
%token <sval> AND ASC CSV FOR INT KEY NOT OFF SET TBL TOP AS BY IF
%token <sval> IN IS OF ON OR TO
...
%type <tbl_name>	table_name
%type <sval> 		opt_alias alias file_path
...
%type <expr> 		expr scalar_expr unary_expr binary_expr ternary_expr function_expr star_expr expr_alias placeholder_expr
...
%type <str_vec>		ident_commalist opt_column_list column_commalist
...
file_path:
		string_literal { $$ = strdup($1->name.c_str()); }
	;
...
column_commalist:
	...
	|	column_commalist ',' IDENTIFIER { $1->push_back($3); $$ = $1; }
	;
...
column_def:
	...
	|	PRIMARY KEY '(' column_commalist ')' { $$ = new ColumnDefinition(*($4), ColumnDefinition::DataType::PRIMARY); }
	|	UNIQUE '(' column_commalist ')' { $$ = new ColumnDefinition(*($3), ColumnDefinition::DataType::UNIQUE); }
	|	INDEX '(' column_commalist ')' { $$ = new ColumnDefinition(*($3), ColumnDefinition::DataType::INDEX); }
	;
...
unary_expr:
	...
	|	expr IS NULL { $$ = Expr::makeOpUnary(Expr::OperatorType::ISNULL, $1); }
	|	expr IS NOT NULL { $$ = Expr::makeOpUnary(Expr::OperatorType::ISNOTNULL, $1); }
...
ternary_expr:
		IF '(' expr ',' expr ',' expr ')' { $$ = Expr::makeOpTernary($3, $5, $7, Expr::OperatorType::IF); }
	;
...
function_expr:
		SUBSTRING '(' expr ',' expr ')' { $$ = Expr::makeFunctionRef(Expr::SUBSTRING, $3, $5); }
	|	SUBSTRING_INDEX '(' expr ',' expr ',' expr ')' { $$ = Expr::makeFunctionRef(Expr::SUBSTRING_INDEX, $3, $5, $7); }
	;
...
table_name:
		IDENTIFIER { $$ = new TableName($1); }
	|	IDENTIFIER '.' IDENTIFIER { $$ = new TableName($1, $3); }
	;
...
join_clause:
		join_clause opt_join_type JOIN join_table{ //( 2 )
	...
	|	join_clause opt_join_type JOIN join_table ON join_condition { //( 2 )
			$$ = new TableRef(kTableJoin);
			$$->join = new JoinDefinition();
			$$->join->type = (JoinType) $2;
			$$->join->right = $4;
...
ident_commalist:
		IDENTIFIER { $$ = new std::vector<std::string>(); $$->push_back(std::string($1)); }
	|	ident_commalist ',' IDENTIFIER { $1->push_back(std::string($3)); $$ = $1; }
	;
